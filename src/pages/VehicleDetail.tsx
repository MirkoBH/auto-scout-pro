...
import { ArrowLeft, Calendar, Fuel, Gauge, MapPin, Cog, Shield, AlertTriangle, Sparkles, Heart, TrendingUp, Star } from "lucide-react";
...
const VehicleDetail = () => {
  ...
  if (loading) return (
    <div className="container max-w-4xl py-8 space-y-6">
      ...
      <div className="space-y-3"><Skeleton className="h-10 w-64" /><Skeleton className="h-6 w-32" /></div>
    </div>
  );
  ...
  return (
    <div className="container max-w-4xl py-8 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        {isBuyer && (
          <Button variant="outline" size="sm" className="rounded-full" onClick={() => toggleFavorite(pub.id)}>
            <Heart className={`mr-1.5 h-4 w-4 ${isFavorite(pub.id) ? "fill-destructive text-destructive" : ""}`} />
            {isFavorite(pub.id) ? "Guardado" : "Guardar"}
          </Button>
        )}
      </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {pub.puntaje != null && (
                      <Badge variant="outline" className="text-xs">
                        <Star className="mr-1 h-3 w-3" /> {pub.puntaje}/100
                      </Badge>
                    )}
                  </div>
                  {pub.estimacion_danos && (
                    <p className="text-sm text-muted-foreground flex items-start gap-1">
                      <AlertTriangle className="h-3.5 w-3.5 text-primary mt-0.5 shrink-0" /> {pub.estimacion_danos}
                    </p>
                  )}
                </div>
              </div>
              {(pub.precio_estimado_min != null || pub.precio_estimado_max != null) && (
                <div className="flex items-center gap-2 pl-16">
                  <TrendingUp className="h-4 w-4 text-primary" />
                  <span className="text-sm font-medium">Precio estimado:</span>
                  <span className="text-sm text-muted-foreground">
                    ${Number(pub.precio_estimado_min || 0).toLocaleString()} – ${Number(pub.precio_estimado_max || 0).toLocaleString()}
                  </span>
                </div>
              )}
            </CardContent>
